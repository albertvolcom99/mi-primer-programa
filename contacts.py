...
def find_contact(contacts, opt):
    ...
    while find_again != "N":
        ...
        if len(contact_indexes) > 1:
            ...
            while find_again!="S" and find_again!="N":
                find_again = input("¿Buscar de nuevo? S/N: ").upper()
            if find_again=="N":
                return
    if opt == "F":
        ...
        print("Se va a eliminar el contacto {}".format(found_contacts[contact_index]["name"]))
        remove_okey = ""
        while remove_okey != "S" and remove_okey != "N":
            remove_okey = input("¿Está de acuerdo? S/N: ").upper()
        if remove_okey == "S":
            contacts.remove(found_contacts[contact_index])
...
def main():
    ...
    while action != ACTION_EXIT:
        if action == ACTION_ADD_CONTACT:
            ...
            find_contact(contacts, "R")
        elif action == ACTION_FIND_CONTACT:
            find_contact(contacts, "F")
